                ...
                <div className='col-sm-12'>
                    ...
                    {skills.map((skill, ind) => {
                        return (<div className='row' key={ind} data-aos="zoom-in-down" data-aos-once="false">
                            <h5 className='fw-bold pt-4'>{skill.category}</h5>
                            {skill.list.map((item, i) => {
                                return (<div key={i} className='col-sm-6 pb-1'>
                                    <div className='d-flex justify-content-between fsize-12 fw-bold'><label>{item.name}</label><label>{item.level}%</label></div>
